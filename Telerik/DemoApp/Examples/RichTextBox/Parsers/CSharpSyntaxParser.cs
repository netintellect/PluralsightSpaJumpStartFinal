...
namespace Telerik.Windows.Examples.RichTextBox.Parsers
{
	internal class CSharpSyntaxParser : SyntaxParser
	{
		...
		protected override Collection<LanguageSyntaxStructure> LoadLanguageSyntax(Collection<LanguageSyntaxStructure> languageSyntax)
		{
			...
			kwrds.Append(@")(?=(\.)|(\s+)))");
            languageSyntax.Add(new LanguageSyntaxStructure(kwrds.ToString(), "keyword", new SolidColorBrush((Color)ColorConverter.ConvertFromString("#0160E5"))));
            ...
            languageSyntax.Add(new LanguageSyntaxStructure(identifier, "identifier", new SolidColorBrush((Color)ColorConverter.ConvertFromString("#000000"))));

			////Load other
			string other = @"\G(?<other>[\s\S]\s*)";
            languageSyntax.Add(new LanguageSyntaxStructure(other, "other", new SolidColorBrush((Color)ColorConverter.ConvertFromString("#000000"))));
