...
namespace Telerik.Windows.Diagrams.Features
{
	...
	public static class FeatureUtilities
	{
        ...
        /// <returns>
        /// </returns>
        private static Stream GetStream(Uri uri, string assemblyName)
        {
            if (uri != null)
            {
                return GetStream(uri.ToString(), assemblyName);
            }
            else
            {
                return null;
		...
		/// <returns>
		/// </returns>
		private static Stream GetStream(string relativeUri, string assemblyName)
		{
			try
			{
