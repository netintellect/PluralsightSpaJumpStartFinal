...
namespace Telerik.Windows.Examples.ChartView.InverseAxis.ViewModels
{
    public class DrillDataViewModel : ViewModelBase
	{
		public List<ResourceTypeData> Data { get; private set; }
		...
		public double SelectionStart
		{
			...
			set 
			{
				if (this.selectionStart != value)
				{
					...
					this.OnPropertyChanged(() => this.SelectionStart);
				}
			}
		}
		...
		public double SelectionEnd
		{
			...
			set
			{
				if (this.selectionEnd != value)
				{
					...
					this.OnPropertyChanged(() => this.SelectionEnd);
				}
			}
		}
		
        public DrillDataViewModel()
        {
			var dryHolesDrillData = GetDryHolesDrillData();
			var naturalGasDrillData = GetNaturalGasDrillData();
			var crudeOilDrillData = GetCrudeOilDrillData();

			var dryHolesData = new ResourceTypeData()
			{
				Name = "Dry holes",
				ItemsSource = dryHolesDrillData,
				AverageDepth = dryHolesDrillData.Average(data => data.Depth), 
				MaximumDepth = dryHolesDrillData.Max(data => data.Depth), 
			};
			
			var naturalGas = new ResourceTypeData()
			{
				Name = "Natural gas",
				ItemsSource = naturalGasDrillData,
				AverageDepth = naturalGasDrillData.Average(data => data.Depth),
				MaximumDepth = naturalGasDrillData.Max(data => data.Depth), 
			};
			
			var crudeOil = new ResourceTypeData()
			{
				Name = "Crude oil",
				ItemsSource = crudeOilDrillData,
				AverageDepth = crudeOilDrillData.Average(data => data.Depth), 
				MaximumDepth = crudeOilDrillData.Max(data => data.Depth), 
			};
			
			this.Data = new List<ResourceTypeData>()
			{
				dryHolesData,
				naturalGas,
				crudeOil,
		...
		private static List<WellDrillData> GetNaturalGasDrillData()
		{
		...
		private static List<WellDrillData> GetDryHolesDrillData()
		{
			return new List<WellDrillData>()
			{
		...
		private static List<WellDrillData> GetCrudeOilDrillData()
		{
			return new List<WellDrillData>()
			{
